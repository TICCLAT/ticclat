...
class Corpus(Base):
    ...
    corpus_documents = relationship(
        "Document", secondary=corpusId_x_documentId, back_populates="document_corpora"
    )
...
class Document(Base):
    ...
    document_corpora = relationship(
        "Corpus", secondary=corpusId_x_documentId, back_populates="corpus_documents"
    )
    document_wordforms = relationship("TextAttestation", back_populates="ta_document")


lexical_source_wordform = Table(
...
class Lexicon(Base):
    ...
    lexicon_wordforms = relationship(
        "Wordform", secondary=lexical_source_wordform, back_populates="wf_lexica"
    )
...
class Wordform(Base):
    ...
    wordform_lowercase = Column(
        Unicode(255, convert_unicode=False), nullable=False, index=True
    )

    wf_lexica = relationship(
        "Lexicon", secondary=lexical_source_wordform, back_populates="lexicon_wordforms"
    )
    ...
    def link_with_metadata(self, wf_to, wf_from_correct, wf_to_correct, lexicon):
    ...
    def link_spelling_correction(self, corr, lexicon):
        ...
        Inputs:
            ...
            lexicon (Lexicon): The Lexicon that contains the WordformLink
        """
        self.link_with_metadata(
            corr, wf_from_correct=False, wf_to_correct=True, lexicon=lexicon
        )
...
class WordformLink(Base):
    ...
    wordform_from = Column(
        BigInteger(), ForeignKey("wordforms.wordform_id"), primary_key=True
    )
    wordform_to = Column(
        BigInteger(), ForeignKey("wordforms.wordform_id"), primary_key=True
    )
    ...
    linked_from = relationship(
        "Wordform", backref="links", primaryjoin=(Wordform.wordform_id == wordform_from)
    )
    linked_to = relationship(
        "Wordform",
        backref="links_2_to_1",
    ...
    def __str__(self):
        return "<WordformLink {} -> {}>".format(
            self.linked_from.wordform, self.linked_to.wordform
        )
...
class WordformLinkSource(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["wordform_from", "wordform_to"],
            ["wordform_links.wordform_from", "wordform_links.wordform_to"],
        ),
    )

    source_x_wordform_link_id = Column(
        BigInteger().with_variant(Integer, "sqlite"), primary_key=True
    )
