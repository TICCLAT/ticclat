...
def load_GB95(path):
    df = pd.read_csv(
        ...
        names=[
            ...
            "article",
            "plural/past/attrib",
            "plural/past/attrib syllables",
            "diminu/compara/past plural",
            "diminu/compara/past plural syllables",
...
def clean_wordform_df(wordform_df):
    ...
    duplicates = wordform_df.str.contains(r"[0-9]$", regex=True)
    wordform_df = pd.concat(
        (
            wordform_df[~duplicates],
            wordform_df[duplicates].str.replace(r"[0-9]$", "", regex=True),
        )
    )
...
def clean_disambiguation_column(df):
    ...
    df["disambiguation"][
        df["disambiguation"].str.contains(" ", na=False) & ~df["disambiguation"].str.contains(",", na=False)
    ] = None
    df["disambiguation"][df["disambiguation"].str.contains(r"[^,] ", na=False)] = (
        df["disambiguation"][df["disambiguation"].str.contains(r"[^,] ", na=False)]
        .str.split(", ")
...
def clean_wordform_series(wordform_series, remove_duplicates=False):
    ...
    # remove duplicate word footnote numbers
    duplicates = wordform_series.str.contains(r"[0-9]$", regex=True)
    wordform_series = pd.concat(
        (
            wordform_series[~duplicates],
            wordform_series[duplicates].str.replace(r"[0-9]$", "", regex=True),
        )
    )
    # remove parentheses around some words
...
if __name__ == "__main__":
    ...
    engine = sqlalchemy.create_engine(
        "mysql://{}:{}@localhost/{}?charset=utf8mb4".format(
            os.environ["user"], os.environ["password"], os.environ["dbname"]
        )
    )
    ...
    with ticclat.dbutils.session_scope(Session) as session:
        ticclat.dbutils.add_lexicon(
            session,
            "Groene Boekje 1995",
            True,
            wordform_clean_df.to_frame(name="wordform"),
        )
    ...
    with ticclat.dbutils.session_scope(Session) as session:
        lexicon = (
            session.query(ticclat.ticclat_schema.Lexicon)
            .filter(ticclat.ticclat_schema.Lexicon.lexicon_name == "Groene Boekje 1995")
            .first()
        )
        if lexicon is None:
        ...
        for idx, row in tqdm.tqdm(link_df.iterrows(), total=link_df.shape[0]):
            wf = (
                session.query(ticclat.ticclat_schema.Wordform)
                .filter(ticclat.ticclat_schema.Wordform.wordform == row["wordform_1"])
                .first()
            )
            corr = (
                session.query(ticclat.ticclat_schema.Wordform)
                .filter(ticclat.ticclat_schema.Wordform.wordform == row["wordform_2"])
                .first()
            )
            if corr is None:
                print(
                    "Wordform '",
                    row["wordform_2"],
                    "' not found in wordforms table, continuing...",
                )
