...
def bulk_add_wordforms(session, wfs, disable_pbar=False, batch_size=10000):
    ...
    if not wfs["wordform"].is_unique:
        logger.info(
            "The wordform-column contains duplicate entries. " "Removing duplicates."
        )
...
def add_lexicon(session, lexicon_name, vocabulary, wfs, batch_size=10000):
    ...
    session.execute(
        ...
        [{"lexicon_id": lexicon_id, "wordform_id": wf["wordform_id"]} for wf in result],
    )
...
def get_wf_mapping(session, lexicon=None, lexicon_id=None):
    ...
    if lexicon is not None:
        if lexicon.lexicon_id is None:
            raise ValueError(
                "The lexicon does not (yet) have an ID. Please"
    ...
    s = select([lexical_source_wordform.join(Lexicon).join(Wordform)]).where(
        Lexicon.lexicon_id == lexicon_id
    )
    logger.debug(s)
    result = session.execute(s).fetchall()
...
def bulk_add_anahashes(session, anahashes, tqdm=None, batch_size=10000):
    ...
    logger.debug(
        f"The input data contains {anahashes.shape[0]} wordform/anahash pairs."
    )
...
def connect_anahashes_to_wordforms(session, anahashes, df, batch_size=50000):
    ...
    with get_temp_file() as anahash_to_wf_file:
        t = write_json_lines(anahash_to_wf_file, get_anahashes(session, anahashes, df))

        u = (
            Wordform.__table__.update()
        ...
        sql_query_batches(
            session, u, read_json_lines(anahash_to_wf_file), t, batch_size
        )
...
def write_wf_links_data(
    ...
    for _, row in tqdm(links_df.iterrows(), total=links_df.shape[0]):
        ...
        if wf_from != wf_to and (wf_from, wf_to) not in wf_links:
            s = select([WordformLink]).where(
                and_(
                    WordformLink.wordform_from == wf_from,
            ...
            if r is None:
                ...
                links_file.write(
                    json_line({"wordform_from": wf_from, "wordform_to": wf_to})
                )
                links_file.write(
                    json_line({"wordform_from": wf_to, "wordform_to": wf_from})
                )
...
def add_lexicon_with_links(
    ...
    lexicon = add_lexicon(
        session, lexicon_name, vocabulary, wordforms, batch_size=batch_size
    )
    ...
    with get_temp_file() as wfl_file:
        ...
        with get_temp_file() as wfls_file:
            ...
            sql_insert_batches(
                session, WordformLink, read_json_lines(wfl_file), batch_size=batch_size
            )
...
def create_ticclat_database(
    delete_existing=False, dbname="ticclat_test", user="", passwd="", host="localhost"
):
    ...
    with db.cursor() as cursor:
        try:
            cursor.execute(
                f"CREATE DATABASE {dbname} CHARACTER SET utf8mb4 COLLATE utf8mb4_bin;"
            )
        except MySQLdb.ProgrammingError as e:
            if database_exists(engine.url):
                if not delete_existing:
                    raise Exception(
                        f"Database `{dbname}` already exists, delete it first before recreating."
                    )
                else:
